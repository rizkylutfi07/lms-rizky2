...
export class AttendanceController {
    ...
    @Get('manual')
    @Roles(Role.ADMIN, Role.GURU, Role.PETUGAS_ABSENSI)
    getStudentsForManualAttendance(@Query() query: ManualAttendanceQueryDto, @Request() req) {
        return this.attendanceService.getStudentsForManualAttendance(query, req.user.userId);
    }
    ...
    updateAttendanceStatus(
        ...
        return this.attendanceService.updateAttendanceStatus(id, dto, req.user.userId);
    }

    @Post('manual')
    @Roles(Role.ADMIN, Role.GURU, Role.PETUGAS_ABSENSI)
    createManualAttendance(@Body() dto: ManualAttendanceDto, @Request() req) {
        return this.attendanceService.createManualAttendance(dto, req.user.userId);
    }
}
