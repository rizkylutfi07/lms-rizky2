...
export class AttendanceService {
    ...
    // Helper method to check if user can edit attendance for a student
    // Returns true if: ADMIN or PETUGAS_ABSENSI only
    private async canEditAttendance(userId: string, kelasId: string | null): Promise<{ canEdit: boolean; reason?: string }> {
        try {
            ...
            if (!user) {
                return { canEdit: false, reason: 'User tidak ditemukan' };
            }

            console.log('DEBUG canEditAttendance:', { userId, role: user.role, isAdmin: user.role === Role.ADMIN, isPetugas: user.role === Role.PETUGAS_ABSENSI });

            // Only ADMIN and PETUGAS_ABSENSI can edit
            if (user.role === Role.ADMIN || user.role === Role.PETUGAS_ABSENSI) {
                return { canEdit: true };
            }

            // GURU (including wali kelas) can only view
            return { canEdit: false, reason: 'Hanya Admin atau Petugas Absensi yang dapat mengubah absensi.' };
        } catch (error) {
            console.error('Error checking edit permission:', error);
            return { canEdit: false, reason: 'Gagal memeriksa akses' };
        }
    }
    ...
    async getStudentsForManualAttendance(query: ManualAttendanceQueryDto, userId?: string) {
        let targetDate: Date;
        ...
        // Check user permission - only ADMIN and PETUGAS_ABSENSI can edit
        let canEdit = false;
        ...
        if (userId) {
            ...
            console.log('DEBUG canEdit:', { userId, userRole: user?.role, canEdit });

            if (user && (user.role === 'ADMIN' || user.role === 'PETUGAS_ABSENSI')) {
                canEdit = true;
            }

            console.log('DEBUG canEdit result:', { canEdit });
        }
        ...
        const result = students.map(student => {
            ...
            return {
                ...
                canEdit: studentCanEdit, // Per-student edit permission
            };
        });
        ...
        return {
            ...
            stats,
            students: result,
            canEdit, // Global edit permission (for UI hints)
        };
    }
    ...
    async updateAttendanceStatus(id: string, dto: UpdateAttendanceDto, userId: string) {
        ...
        const permission = await this.canEditAttendance(userId, attendance.siswa?.kelasId || null);
        if (!permission.canEdit) {
            throw new BadRequestException(permission.reason || 'Anda tidak memiliki akses untuk mengubah absensi');
        }
    ...
    async createManualAttendance(dto: ManualAttendanceDto, userId: string) {
        ...
        // Check if user has permission to edit attendance
        const permission = await this.canEditAttendance(userId, siswa.kelasId);
        if (!permission.canEdit) {
            throw new BadRequestException(permission.reason || 'Anda tidak memiliki akses untuk mengubah absensi');
        }
