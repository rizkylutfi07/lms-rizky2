...
export class UjianSiswaService {
    ...
    async getCompletedExams(siswaId: string) {
        const ujianSiswa = await this.prisma.ujianSiswa.findMany({
            where: {
                siswaId,
    ...
    async logActivity(logDto: LogActivityDto, siswaId: string) {
        ...
        const ujianSiswa = await this.prisma.ujianSiswa.findFirst({
            ...
            // Include violationCount so we know the current persistent count
        }) as any;
        ...
        if (isViolation) {
            ...
            dbViolationCount = updated.violationCount;
        }
        ...
        const violationCount = Math.max(dbViolationCount, memViolationCount);
