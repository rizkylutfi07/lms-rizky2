...
export const materiApi = {
    ...
    getOne: (id: string, incrementView: boolean = true) =>
        fetchApi(`/materi/${id}?incrementView=${incrementView}`),
    ...
    updateWithFile: async (id: string, formData: FormData) => {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                error = { message: errorText || 'Update failed' };
            }
...
// Absensi Kelas API
export const absensiKelasApi = {
    getMyToday: () =>
        fetchApi('/absensi-kelas/my-today'),

    getStudentsForJadwal: (jadwalId: string) =>
        fetchApi(`/absensi-kelas/jadwal/${jadwalId}/students`),

    getOne: (id: string) =>
        fetchApi(`/absensi-kelas/${id}`),
    ...
    getAll: (params?: {
        ...
        fetchApi(`/absensi-kelas?${new URLSearchParams(params as any).toString()}`),

    create: (data: any) =>
        fetchApi('/absensi-kelas', {
            method: 'POST',
            body: JSON.stringify(data),
        }),
    ...
    getRekap: (params?: {
        ...
        fetchApi(`/absensi-kelas/rekap?${new URLSearchParams(params as any).toString()}`),
};
