...
export default function TugasManagementPage() {
    ...
    const { data: tugasList = [], isLoading } = useQuery({
        ...
        queryFn: async () => (await tugasApi.getAll({ myAssignments: "true" })) as any[],
    });
    ...
    const mataPelajaranList = Array.isArray(mataPelajaranResponse)
        ...
        : ((mataPelajaranResponse as any)?.data || []);
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                {role === "ADMIN" ? (
                                                    formData.guruId && (selectedGuru as any)?.mataPelajaran ? (
                                                        <SearchableSelect
                                                            options={(selectedGuru as any).mataPelajaran.map((mp: any) => ({
                                                                value: mp.id,
                                                                label: mp.nama,
                                                                description: mp.kode,
                                                            }))}
...
function SubmissionsView({
    ...
    const { data: submissions = [], isLoading } = useQuery({
        ...
        queryFn: async () => (await tugasApi.getSubmissions(tugasId)) as any[],
        enabled: !!tugasId,
    });
