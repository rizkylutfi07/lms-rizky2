...
function getKelasInitial(nama: string): string {
    ...
    // If no words left after filtering, return first 2 chars of original
    if (words.length === 0) {
...
export default function JadwalPelajaranPage() {
    ...
    const getCurrentDay = (): string => {
        ...
        const currentDay = days[today] ?? "SENIN";
                                    ...
                                    {timeSlots.map((slot, rowIndex) => (
                                        <tr key={`${slot.mulai}-${slot.selesai}`} className={slot.isBreak ? "bg-amber-500/10" : ""}>
                                            ...
                                            {classList.map((kelas: any) => {
                                                ...
                                                const [startHour = 0, startMin = 0] = slot.mulai.split(':').map(Number);
                                                const [endHour = 0, endMin = 0] = slot.selesai.split(':').map(Number);
