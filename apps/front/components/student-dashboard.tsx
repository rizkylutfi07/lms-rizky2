...
const getCurrentDay = (): string => {
    ...
    return currentDay === "MINGGU" ? "SENIN" : (currentDay || "SENIN");
};
...
export default function StudentDashboard() {
    ...
    const startDate = getLocalDateString(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1));
    const endDate = getLocalDateString(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0));
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col gap-1 items-end">
                                                    ...
                                                    <Badge className="hidden sm:flex border">
                                                        Jam ke-{index + 1}
                                                    </Badge>
                                                </div>
                                            </div>
                                    ...
                                    {upcomingTasks.map((task: any) => {
                                        ...
                                        return (
                                            <div key={task.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-lg border bg-background/50 p-4 hover:border-emerald-500/30 transition-all">
                                                ...
                                                <Badge
                                                    className={cn("w-fit", daysInfo.urgent ? "bg-red-500/10 text-red-600 hover:bg-red-500/20" : "bg-muted text-muted-foreground")}
                                                >
                            ...
                            <div className="grid grid-cols-7 gap-1">
                                ...
                                {calendarDays.map((date, index) => {
                                    ...
                                    const statusColors: Record<string, string> = {
                                        HADIR: "bg-green-500/20 text-green-700 dark:text-green-400 border-green-500/40",
                                        TERLAMBAT: "bg-orange-500/20 text-orange-700 dark:text-orange-400 border-orange-500/40",
                                        SAKIT: "bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-500/40",
                                        IZIN: "bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 border-yellow-500/40",
                                        ALPHA: "bg-red-500/20 text-red-700 dark:text-red-400 border-red-500/40",
                                    };
                                    ...
                                    return (
                                        <div
                                            key={dateStr}
                                            className={cn(
                                                "aspect-square rounded text-xs flex flex-col items-center justify-center font-medium border",
                                                isToday && "ring-2 ring-primary",
                                                attendance ? statusColors[attendance.status] : "text-muted-foreground/50 border-transparent"
                                            )}
                                            title={attendance ? `${date.getDate()} - ${attendance.status}` : date.getDate().toString()}
                                        >
                                            <span className="text-[10px] font-semibold">{date.getDate()}</span>
                                            {StatusIcon && (
                                                <StatusIcon size={10} className="mt-0.5" />
                                            )}
                                        </div>
                                    );
                                })}
